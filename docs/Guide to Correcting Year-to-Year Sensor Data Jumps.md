...
_(You can insert your "RM 54.0 Comparison (Series 27 Sensor 1 - Updated).jpg" here or a similar before/after
visualization)_
...
- **Year-to-Year Differences Summary:** A summary file (specifically, the CSV exported from your analysis workbook, like
  ...
  process _where_ the potentially problematic jumps are located (which year transition and which sensor).
...
- **Step 2: Fix the Data (Automated Refined Level Shift Correction)**
  ...
  - This calculated Level Shift is applied by adding it to _every single reading_ for that sensor throughout the
...
- **Step 3: Get the New Averages (Automated Summary Calculation)**
  - A third Python script (let's call it `calculate_updated_averages.py`) reads the _new, corrected_ data files
    created in Step 2.
...
valid readings of the next year, it figures out the _exact_ shift needed to make those two points meet smoothly.
...
- **Always keep your original raw data files safe and untouched.** The automated correction process creates _new_
