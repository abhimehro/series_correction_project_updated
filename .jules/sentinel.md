...
**Vulnerability:** In an attempt to fix Information Exposure (CWE-209), internal logging mechanisms (`log.exception`) were downgraded to `log.error` and exception objects were stripped out entirely, destroying observability.
**Learning:** It is crucial to prevent stack traces from leaking in HTTP responses or to untrusted end-users via CLI output. However, removing stack traces or exception details from an application's internal logs constitutes "security theater" that actively makes the application much worse to operate and debug.
**Prevention:** Keep full error tracebacks in internal logs via `log.exception(...)`, but ensure user-facing outputs get sanitized, generic error messages. Furthermore, `log.exception` automatically captures stack traces; changing the message string does not stop the stack trace from being logged.
