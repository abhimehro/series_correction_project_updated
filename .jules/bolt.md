...
## 2025-07-06 - Avoid redundant rolling window calculations
**Learning:** Inside chunked loops that calculate statistics over sliding windows (like `sliding_window_view`), it is extremely inefficient to redundantly compute the rolling median (e.g., `np.nanmedian(cw, axis=1)`) if a full array `rolling_median` has already been precomputed.
**Action:** When a global rolling statistic (like median) is available, reuse it by slicing the precomputed array directly. For a `center=True` pandas rolling window, the median aligns exactly at index `pad = window_size // 2` within the sliding window, so you can slice the precomputed array as `rolling_median[start_idx + pad : end_idx + pad]` for an immediate $O(1)$ lookup instead of an $O(N \cdot W)$ calculation.
