...
## 2025-02-20 - Avoid Pandas .rolling().median() overhead with NumPy sliding_window_view
**Learning:** Using Pandas `.rolling(window=window_size).median()` creates significant overhead, especially in performance-critical anomaly detection loops.
**Action:** Replace it with NumPy's `sliding_window_view` over a padded array, then apply `np.nanmedian` across `axis=1`. Ensure you replicate Pandas `min_periods` behavior by counting NaNs per window and setting the result to `np.nan` if any exist. Also ensure that the original array is cast to float (`astype(float)`) before padding with `np.nan` to avoid `ValueError: cannot convert float NaN to integer`. Always add the necessary imports (`from numpy.lib.stride_tricks import sliding_window_view` and `import warnings`) globally at the top of the file, strictly below any `from __future__` imports.
