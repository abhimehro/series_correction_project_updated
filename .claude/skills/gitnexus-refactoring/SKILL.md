...
`detect_changes` takes `worktree` when you are editing a linked worktree the MCP
server was not launched from; otherwise `git diff` runs in the wrong checkout
and reports nothing changed, which reads as a verified refactor.
...
A wrong-worktree zero carries neither flag and is indistinguishable from a clean
verification, so confirm the diffed checkout is the one you edited.
