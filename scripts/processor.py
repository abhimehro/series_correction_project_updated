...
def process_data(
    ...
    processed_data = _process_discontinuity(
        ...
        detect_gaps,
        correct_gaps,
        {"time_col": time_col, "threshold_factor": gap_threshold_factor},
        {"time_col": time_col, "value_cols": [value_col], "method": gap_method},
    ...
    processed_data = _process_discontinuity(
        ...
        detect_outliers,
        correct_outliers,
        {"value_col": value_col, "window_size": window_size, "threshold": threshold},
        {"value_col": value_col, "window_size": window_size, "method": outlier_method},
    )
    processed_data = _process_discontinuity(
        processed_data,
        ...
        correct_jumps,
        {"value_col": value_col, "window_size": window_size, "threshold": threshold},
        {"value_col": value_col, "window_size": window_size},
    )
...
def _process_discontinuity(
    ...
    indices = detect_func(processed_data, **detect_kwargs)
    if indices:
        processed_data = correct_func(processed_data, indices, **correct_kwargs)
        if sort_time_col:
            processed_data = processed_data.sort_values(by=sort_time_col).reset_index(
                drop=True
            )
    else:
        log.info(f"No {step_name.split(' ')[-1].lower()} detected or corrected.")
    return processed_data
