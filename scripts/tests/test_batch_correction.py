...
class TestBatchProcess:
    """Test suite for batch processing flow and options."""
    ...
    def test_batch_process_happy_path_specific_series_no_config(self, mock_dependencies):
        config_mock = {
            "RAW_DATA_DIR": "/fake/data/dir",
            "RIVER_MILE_MAP_PATH": "scripts/river_mile_map.csv",
        }
    ...
    def test_batch_process_config_not_found(self, mock_dependencies, mock_config_loader, caplog):
...
class TestDetermineSeries:
    """Test suite for series selection logic."""
    ...
    def test_determine_series_to_process_all_fallback_with_river_miles(self, mocker, tmp_path):
...
class TestGetDirectoryAndRawData:
    """Test suite for filesystem directory retrieval and file loading."""
...
class TestEnrichConfig:
    """Test suite for config enrichment and validation."""

    def test_enrich_config_with_river_mappings_path_traversal(self, caplog):
    ...
    def test_enrich_config_with_river_mappings_commonpath_error(self, caplog, mocker):
        caplog.set_level("WARNING")
        config_data = {"RIVER_MILE_MAP_PATH": "scripts/river_mile_map.csv"}
        mocker.patch("os.path.commonpath", side_effect=ValueError("Paths on different drives"))
