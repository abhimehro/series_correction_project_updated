...
def patch_pd_read_csv(monkeypatch):
    def read_csv_side_effect_pd(path, *_args, **_kwargs):
    ...
    monkeypatch.setattr("scripts.batch_correction.pd.read_csv", read_csv_side_effect_pd)
...
class TestBatchProcessHappyPath:
    ...
    def test_happy_path_all_series_with_config(self, mock_dependencies):
        config_mock = {
            "RAW_DATA_DIR": "/fake/data/dir",
            "RIVER_MILE_MAP_PATH": "scripts/river_mile_map.csv",
        }
    ...
    def test_happy_path_specific_series_no_config(self, mock_dependencies):
        config_mock = {
            "RAW_DATA_DIR": "/fake/data/dir",
            "RIVER_MILE_MAP_PATH": "scripts/river_mile_map.csv",
        }
...
class TestBatchProcessErrorHandling:
    """Tests for error handling in batch processing."""

    def test_dry_run(self, mock_dependencies, mock_config_loader):
    ...
    def test_no_files_found(self, mock_dependencies, mock_config_loader):
        ...
        # Act: no matching S99 files
        summary_df = batch_process(
            BatchConfig(series_selection, river_miles, years, dry_run=dry_run)
        )
        assert summary_df.empty

    def test_data_dir_not_found(self, mock_dependencies):
    ...
    def test_skip_empty_file(self, mock_dependencies, caplog):
    ...
    def test_load_error(self, mock_dependencies, mock_config_loader, caplog, mocker):
    ...
    def test_config_not_found(self, mock_dependencies, mock_config_loader, caplog):
        ...
        warning_logged = any(
            "not found" in record.message and "continuing with empty config" in record.message
            for record in caplog.records
            if record.levelname == "WARNING"
        )
        assert warning_logged

    def test_fallback_mode_exception(
        self,
        mock_dependencies,
        mock_config_loader,
        mocker,
        ...
        summary_df = bc.batch_process(
            bc.BatchConfig(series, None, years, dry_run=False)
        )
...
class TestSeriesDetermination:
    """Tests for series determination functionality."""
...
class TestBatchProcessWithProcessor:
    """Tests for batch processing with processor module."""
