...
class TestBatchProcessMain:
    """Tests for the primary batch_process workflows and configurations."""

    def test_batch_process_happy_path_all_series_with_config(self, mock_dependencies):
        config_mock = {
            "RAW_DATA_DIR": "/fake/data/dir",
            "RIVER_MILE_MAP_PATH": "scripts/river_mile_map.csv",
        }
        ...
        with patch(
            ...
            years = (1995, 1996)
            dry_run = False
            expected_data_dir_inner = "/fake/data/dir"
            ...
            with patch("pandas.read_csv", side_effect=_read_csv_side_effect_all_series):
                summary_df = bc.batch_process(
                    bc.BatchConfig(
                        series_selection, river_miles, years, dry_run=dry_run
                    )
                )
            ...
            assert all(
                status in valid_statuses for status in summary_df["Status"].tolist()
            )
    ...
    def test_batch_process_happy_path_specific_series_no_config(
    ...
    def test_batch_process_dry_run(self, mock_dependencies, mock_config_loader):
        series_selection = "all"
        river_miles = [54.0]
        years = (1995, 1995)
        dry_run = True
    ...
    def test_minimal_happy_path(self, monkeypatch):
        ...
        def _glob_side_effect(pat):
            ...
            return [
                os.path.join(dirn, f) for f in file_list if fnmatch.fnmatch(f, base)
            ]
        ...
        monkeypatch.setattr(
            "scripts.loaders.load_config", lambda path=None: config_mock
        )
        ...
        monkeypatch.setattr(
            "pandas.DataFrame.to_excel", lambda self, path, **kwargs: None
        )
...
class TestBatchProcessEdgeCases:
    """Tests for edge cases, missing inputs, and fallback processing modes."""

    def test_batch_process_no_files_found(self, mock_dependencies, mock_config_loader):
        series_selection = 99
        river_miles = None
        years = (2000, 2001)
        dry_run = False
    ...
    def test_batch_process_skip_empty_file(self, mock_dependencies, caplog):
    ...
    def test_batch_process_fallback_mode_exception(
        self, mock_dependencies, mock_config_loader, mocker
    ):
        series = 26
        years = (1995, 1995)
        ...
        mocker.patch(
            "scripts.batch_correction._load_raw_data",
            side_effect=Exception("Load failed"),
        )

        summary_df = __import__(
            "scripts.batch_correction"
...
class TestDirectoryAndConfigResolution:
    """Tests for directory discovery and configuration loading."""

    def test_batch_process_data_dir_not_found(self, mock_dependencies):
        series_selection = "all"
        river_miles = None
        years = (2000, 2001)
        dry_run = False
        ...
        expected_data_dir_inner = os.path.join(os.getcwd(), "data")
        with pytest.raises(
    ...
    def test_get_data_directory_creates_dir(self, mock_dependencies):
        config_data = {}
    ...
    def test_get_data_directory_creates_dir_oserror(self, mock_dependencies):
        config_data = {}
        with patch("os.path.isdir", return_value=False), patch(
    ...
    def test_batch_process_config_not_found(
        self, mock_dependencies, mock_config_loader, caplog
    ):
...
class TestSeriesAndDataLoader:
    """Tests for series selection logic and raw data loading."""

    def test_batch_process_invalid_series_selection(self, monkeypatch):
    ...
    def test_load_raw_data_empty_file(self, caplog):
    ...
    def test_determine_series_to_process_all_fallback(self, mocker, tmp_path):
        data_dir = tmp_path / "data"
        data_dir.mkdir()
        (data_dir / "S1_Y1.txt").touch()
        (data_dir / "S2_Y1.txt").touch()
        (data_dir / "S3.txt").touch()
        (data_dir / "Sinvalid_Y1.txt").touch()

        series = _determine_series_to_process("all", None, {}, str(data_dir))
        assert series == [1, 2]

    def test_determine_series_to_process_all_fallback_with_river_miles(
    ...
    def test_determine_series_to_process_invalid_sensor_id_in_map(self, mocker):
        mock_log = mocker.patch("scripts.batch_correction.log")
    ...
    def test_determine_series_to_process_explicit_invalid_value(self, mocker):
        mock_log = mocker.patch("scripts.batch_correction.log")
        with pytest.raises(ValueError, match="Invalid series selection"):
...
class TestSecurityValidation:
    """Security tests for config and path traversal validation."""

    def test_enrich_config_river_mile_map_path_traversal(self, caplog):
    ...
    def test_enrich_config_river_mile_map_invalid_path_value_error(
        self, caplog, mocker
    ):
