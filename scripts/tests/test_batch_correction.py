...
class TestBatchCorrectionHappyPath:
...
class TestBatchCorrectionErrors:
    ...
    def test_batch_process_data_dir_not_found(self, mock_dependencies):
        ...
        with pytest.raises(FileNotFoundError, match="Default data directory not found"):
            batch_process(
                BatchConfig(series_selection, river_miles, years, dry_run=dry_run)
            )
        expected_calls = [
...
class TestBatchCorrectionOther:
