...
def _convert_time_col_to_numeric(processed_data, time_col):
    try:
...
def _validate_and_convert_time_col(processed_data, time_col):
    ...
    if not pd.api.types.is_numeric_dtype(processed_data[time_col]):
        return _convert_time_col_to_numeric(processed_data, time_col)
    return processed_data
...
def _auto_detect_value_col(processed_data, time_col):
    ...
    log.info("Auto-detected value column: '%s'", potential_value_cols[0])
...
def _process_discontinuity(processed_data, config: DiscontinuityConfig):
