...
def main():
    try:
        ...
        if not sensor_cols:
            print(
                "Error: Could not find sensor columns (e.g., 'Sensor 01' to 'Sensor 32') in the Year-to-Year Differences CSV."
            )
            identified_outliers_df = (
                pd.DataFrame()
            )  # Empty df to prevent script from running
        elif "Year_Pair" not in actual_cols:
            print(
                f"Error: 'Year_Pair' column not found in the Year-to-Year Differences CSV. Available columns: {actual_cols}"
            )
            ...
            if identified_outliers_df.empty:
                print(
                    "No outliers (|Difference| >= 0.1) found in the Year-to-Year Differences CSV. No corrections will be applied."
                )
            else:
                print(
                    f"Successfully loaded {len(identified_outliers_df)} outliers from {YTY_DIFF_CSV_PATH}"
                )

    except FileNotFoundError:
        print(
            f"Error: The file '{YTY_DIFF_CSV_PATH}' was not found. Please ensure it's in the correct location."
        )
        identified_outliers_df = pd.DataFrame()  # Empty df to prevent script from running
    except Exception as e:
        print(
            f"An error occurred while loading or processing the Year-to-Year Differences CSV: {e}"
        )
        identified_outliers_df = pd.DataFrame()  # Empty df
    ...
    if not identified_outliers_df.empty:
        ...
        for idx, outlier_row in identified_outliers_df.iterrows():
            ...
            original_difference = (
                outlier_row["Difference"]
            )  # Difference from summary file

            # Parse the Year_Pair string (e.g., "1996 (Y02) to 1995 (Y01)")
            pair_match = re.match(
                r"(\d+) \(Y(\d+)\) to (\d+) \(Y(\d+)\)", outlier_year_pair_str
            )
            if not pair_match:
                continue
            ...
            try:
                ...
                if not 0 <= sensor_col_index < 32:
                    print(
                        f"Skipping outlier with invalid sensor column index for {outlier_sensor}."
                    )
                    continue
            except ValueError:
                print(
                    f"Skipping outlier with unparseable sensor number from {outlier_sensor}."
                )
                continue
            ...
            for s_id in ["S26", "S27"]:
                if (
                    ...
                    and prev_year_yy_pair in raw_file_map[s_id]
                    and next_year_yy_pair in raw_file_map[s_id]
                ):
            ...
            if not outlier_series_id:
                print(
                    f"Could not find raw data files for outlier {outlier_year_pair_str}, {outlier_sensor}. Skipping correction."
                )
            ...
            try:
                ...
                df_prev_raw = pd.read_csv(
                    prev_year_raw_file, header=None, sep=r"\s+", engine="python"
                )
                df_next_raw = pd.read_csv(
                    next_year_raw_file, header=None, sep=r"\s+", engine="python"
                )
                ...
                if (
                    ...
                    prev_tail_5_avg_non_zero = calculate_non_zero_average(
                        df_prev_raw.iloc[-5:, sensor_col_index]
                    )

                    # Calculate the average of the first 5 non-zero rows of the next year (from current file)
                    next_head_5_avg_non_zero = calculate_non_zero_average(
                        df_next_raw.iloc[:5, sensor_col_index]
                    )
                    ...
                    df_next_corrected[sensor_col_index] = (
                        pd.to_numeric(
                            df_next_corrected[sensor_col_index], errors="coerce"
                        )
                        + calculated_level_shift
                    )
                    ...
                    applied_corrections.append(
                        {
                            ...
                            "File_Corrected": os.path.basename(
                                next_year_raw_file
                            ).replace(".txt", "_refined_corrected.csv"),
                    ...
                    output_file_name = os.path.basename(next_year_raw_file).replace(
                        ".txt", "_refined_corrected.csv"
                    )
                    output_file_path = os.path.join(
                        CORRECTED_OUTPUT_DIR, output_file_name
                    )
                    ...
                    print(
                        f"  Not enough data points (at least 5) or incorrect column index ({sensor_col_index}) in raw files for {outlier_year_pair_str}, {outlier_sensor}. Skipping correction for this outlier."
                    )

            except FileNotFoundError:
                print(
                    f"  Error: Raw data file not found for {outlier_year_pair_str}, {outlier_sensor}."
                )
            except Exception as e:
                print(
                    f"  An error occurred while processing outlier {outlier_year_pair_str}, {outlier_sensor}: {e}"
                )
