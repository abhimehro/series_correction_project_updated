...
def validate_columns(actual_cols):
    sensor_cols = [
        col
...
def load_identified_outliers(csv_path):
    try:
        ...
        sensor_cols = validate_columns(df_yty_diff.columns.tolist())
        if not sensor_cols:
            return pd.DataFrame()
...
def parse_outlier_info(outlier_year_pair_str, outlier_sensor):
    ...
    prev_year_yy_pair, next_year_yy_pair = (
        (year1_yy, year2_yy) if year1_full < year2_full else (year2_yy, year1_yy)
    )
...
def process_outlier_row(outlier_row, raw_file_map, output_dir):
    ...
    try:
        shift, out_name = apply_correction(
            prev_year_raw_file, next_year_raw_file, sensor_col_index, output_dir
        )
...
def main():
    ...
    if not identified_outliers_df.empty:
        ...
        for outlier_row in identified_outliers_df.itertuples(index=False):
            result = process_outlier_row(outlier_row, raw_file_map, CORRECTED_OUTPUT_DIR)
            if result:
