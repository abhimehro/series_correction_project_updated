...
def _build_rm_to_sensors_map(sensor_to_rm_map: dict, rm_map_key: str = "SENSOR_TO_RIVER") -> dict:
    rm_to_sensors_map = {}
...
def _is_series_filename(fname: str) -> bool:
    return fname.startswith("S") and "_Y" in fname and fname.endswith(".txt")


def _scan_data_dir_for_series(data_dir: str) -> list[int]:
    found = set()
    for fname in os.listdir(data_dir):
        if _is_series_filename(fname):
            try:
...
def _get_series_from_all(
    ...
    if sensor_to_rm_map:
        ...
        log.info("Selecting every series in SENSOR_TO_RIVER map: %s", series_list)
        return series_list

    series_list = _scan_data_dir_for_series(data_dir)
    if river_miles:
...
def _determine_series_to_process(
    ...
    rm_to_sensors_map = _build_rm_to_sensors_map(sensor_to_rm_map, rm_map_key)
