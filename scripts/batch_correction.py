...
def batch_process(
    ...
    if not files_to_process:
        ...
        summary_records = _fallback_process(series_to_process, config_data, output_dir, dry_run)
...
def _fallback_process(series_to_process, config_data, output_dir, dry_run):
    """Perform fallback processing when no standard files are matched."""
    summary_records = []
    ...
    for series_id in series_to_process:
        records = _fallback_process_series(
            series_id, str(series_id), series_config, config_data, output_dir, dry_run
        )
        summary_records.extend(records)
    return summary_records


def _fallback_process_series(series_id, str_series_id, series_config, config_data, output_dir, dry_run):
    summary_records = []
